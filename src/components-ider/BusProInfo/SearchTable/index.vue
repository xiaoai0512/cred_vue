        ...
        <!-- @row-click="chooseCurrClick" -->
            <el-table-column width="55" label="选择">
                <template slot-scope="scope" >
                    <el-radio v-model="currValRadio" :label="scope.row[queryObj.radioKey]">
            ...
            <el-table-column v-for="(item, index) in tableLabel" :key = index :width="item.width" :label="item.label" align="center" >
                <template slot-scope="scope">
                    <span  v-if="item.concat" >
                        {{ scope.row[item.concat.split("_")[0]] }}
                        {{ scope.row[item.concat.split("_")[1]] }}
                    </span>
...
export default {
  ...
  methods: {
    ...
    getList() {
        ...
        if(this.url === '#'){ // 不走接口
            ...
            if (this.codeStr === 'FTYP') { // 费用收取方式
                this.$http.get('/json/feeType.json').then((res) => {
                    this.tableData = res.data.returnData.rows
                });
                // this.tableData = [{
                //     "feeTpyeDesc": "一次性收取",
                //     "feeTpyeCode": "0"
            ...
            if (this.codeStr === 'TERM') { // 期数
                this.$http.get('../../json/term.json').then((res) => {
                    this.tableData = res.data.returnData.rows
                });
                // this.tableData = [{
                //     "termNo": "3"
    ...
    // 
    handleCurrChange(row) {
        this.currValRadio = row[this.queryObj.radioKey]
        this.$emit('choseCuurRow', row)
    },
    chooseCurrClick(row) {
        this.currValRadio = row[this.queryObj.radioKey]
        this.$emit('chooseCurrClick', row)
    },
    handleSearch() {
