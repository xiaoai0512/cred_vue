    ...
    <!-- 表单验证demo: -->
        ...
        <div v-show="showTable">
			...
			<el-table  v-loading = "config.loading" :data="tableData"  class="mbtm20"   fit highlight-current-row style="width: 100%;" border>
                <el-table-column prop="houseOwnerDesc" label="居住状况 " align="center" />
                <el-table-column prop="occupancyTime" label="居住时间" align="center" />
                <el-table-column prop="monthlyMortgagePayment" label="月支付金额" align="center" />
        ...
        <el-dialog title="新增" :visible.sync="dialogFormVisibleAdd" width="65%">
            <el-form ref="refAdd" :model="tempAdd" :rules="rulesAdd">
              	<el-form-item class="cl_td" label="居住状况" prop="houseOwner"
               		:rules="[ { required: true, message: '居住状况不能为空'}]">
	                <el-select clearable  v-model="tempAdd.houseOwner" class="wd200">
	                    <el-option v-for="item in houseOwnerOptions" 
	                    	:key="item.codes" 
	                    	:label="item.detailDesc" 
	                    	:value="item.codes" />
	                </el-select>
	            </el-form-item>
               	<el-form-item class="cl_td" label="居住时间" prop="occupancyTime"
                	:rules="[ { required: true, message: '居住时间不能为空'}]">
                    <el-input v-model="tempAdd.occupancyTime" class="wd200" type="text" maxlength="10"/>
                </el-form-item>
                <el-form-item class="cl_td" label="月支付金额" prop="monthlyMortgagePayment">
                    <el-input v-model="tempAdd.monthlyMortgagePayment" class="wd200" type="text" maxlength="4"/>
                </el-form-item>
            </el-form>
        ...
        <el-dialog title="修改车辆信息" :visible.sync="dialogFormVisibleUp" width="65%">
            <el-form ref="refUp" :model="tempUp" :rules="rulesUp">
          		<el-form-item class="cl_td" label="居住状况" prop="houseOwner"
               		:rules="[ { required: true, message: '居住状况不能为空'}]">
	                <el-select clearable  v-model="tempUp.houseOwner" class="wd200">
	                    <el-option v-for="item in houseOwnerOptions" 
	                    	:key="item.codes" 
	                    	:label="item.detailDesc" 
	                    	:value="item.codes" />
               	...
               	<el-form-item class="cl_td" label="居住时间" prop="occupancyTime"
                	:rules="[ { required: true, message: '居住时间不能为空'}]">
                    <el-input v-model="tempUp.occupancyTime" class="wd200" type="text" maxlength="4"/>
                </el-form-item>
                <el-form-item class="cl_td" label="月支付金额" prop="monthlyMortgagePayment">
                    <el-input v-model="tempUp.monthlyMortgagePayment" class="wd200" type="text" maxlength="10"/>
                </el-form-item>
            </el-form>
            <div slot="footer" class="dialog-footer">
  ...
  components: {
    ...
    Pagination,
  	...
  	mounted() {
	    ...
	    // 居住状况
	    this.getSelectDict('dic_houseOwner').then((res) => {
	        this.houseOwnerOptions = res
	    })
  	},
  	methods: {
  	 	// 查询按钮权限
            ...
            this.getTableList('/beta/betaService/COS.CS.01.0030', params ,(res) => {
                if (res.returnCode == '000000') {
                    if (res.returnData && res.returnData.length > 0 ) {
                       ...
                       if (this.btnAuthStr.includes('BSS.UP.01.0133') != -1) { // 维护
	    ...
	    getList() {
	        ...
	        let params = {
				...
				transDict : ['houseOwner'],//翻译前后key
	        }
	       params = Object.assign( params, this.config, this.queryFormObj)
	        this.getTableList('/nonfi/nonfinanceService/BSS.IQ.01.0133', params ,(res) => {
	            if (res.returnCode == '000000') {
	    ...
	    createData() {
	    	...
	    	this.$refs['refAdd'].validate((valid) => {
	        	if (valid) {
		           	this.post('/nonfi/nonfinanceService/BSS.AD.01.0133', this.tempAdd, (res) => {
		                if (res.returnCode == '000000') {
		                   	this.$notify({
		                        title: 'Success',
		                        message: '新增成功',
    	...
    	// 确定 修改
    	updateData() {
	       this.post('/nonfi/nonfinanceService/BSS.UP.01.0133', this.tempUp, (res) => {
	            if (res.returnCode == '000000') {
	               	this.$notify({
	                    title: 'Success',
	                    message: '修改成功',
