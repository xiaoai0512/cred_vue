...
<template>
    <div class="allContent">
        <el-form ref="queryFormRef" :model="queryForm">
            ...
            <el-form-item class="cl_td" label="账单月" prop="billDate">
                <el-date-picker
        ...
        <div v-show="showInfo">
            <v-table
                :tableData="bill_tableData"
                :tableLabel="bill_tableLabel"
                :config="bill_config"
                :btns="bill_btns"
                :btnWidth="100"
                @event1="bill_handleDetail"
                @changePage="getbillList"
            ></v-table>
        </div>

        <!-- 账单摘要（产品级）弹窗 -->
        <el-dialog title="账单摘要（产品级）" :visible.sync="billMain_dialog"  width="85%">
            <el-form :model="billMainObj">
                <el-form-item class="cl_td bnone" label="客户代码 " prop="customerNo">
                    <el-input v-model="billMainObj.customerNo" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="宽限余额" prop="allGraceBalance">
                    <el-input v-model="billMainObj.allGraceBalance" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="本期贷方发生笔数" prop="creditNum">
                    <el-input v-model="billMainObj.creditNum" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="本期贷方发生额" prop="creditAmount">
                    <el-input v-model="billMainObj.creditAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="借方笔数" prop="debitNum">
                    <el-input v-model="billMainObj.debitNum" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="借方发生额" prop="debitAmount">
                    <el-input v-model="billMainObj.debitAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="本期还款金额" prop="currentRepayAmount">
                    <el-input v-model="billMainObj.currentRepayAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="账单金额" prop="postingAmount">
                    <el-input v-model="billMainObj.postingAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="本金金额" prop="principalAmount">
                    <el-input v-model="billMainObj.principalAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="费用金额" prop="feeAmount">
                    <el-input v-model="billMainObj.feeAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="利息金额" prop="interestAmount">
                    <el-input v-model="billMainObj.interestAmount" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="当期需还欠款" prop="currentAccountBalance">
                    <el-input v-model="billMainObj.currentAccountBalance" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="期初余额" prop="beginBalance">
                    <el-input v-model="billMainObj.beginBalance" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="最低还款额汇总" prop="minRepayAmt">
                    <el-input v-model="billMainObj.minRepayAmt" class="wd200" type="text" />
                </el-form-item>
            </el-form>
            <div class="text_title">延滞信息</div>
            <el-table
                :data="delay_tableData"
                class="mbtm20"
                fit
                ...
                border
            >
                <el-table-column type="index" label="顺序" align="center" />
                <el-table-column prop="transCurrDesc" label="延滞层级" align="center">
                    <template slot-scope="{row}">
                        <span v-if="row.delinquencyLevel=='A'">业务类型级</span>
                        <span v-if="row.delinquencyLevel=='P'">业务项目级</span>
                        <span v-if="row.delinquencyLevel=='G'">业务项目级</span>
                    </template>
                </el-table-column>
                <el-table-column prop="transAmount" label="业务项目" align="center" >
                    <template slot-scope="{row}">
                        <span v-if="row.delinquencyLevel=='A'">{{row.levelCode}}{{row.businessDesc}}</span>
                        <span v-if="row.delinquencyLevel=='P'">{{row.levelCode}}{{row.productLevelCodeDesc}}</span>
                        <span v-if="row.delinquencyLevel=='G'">{{row.levelCode}}{{row.programDesc}}</span>
                    </template>
                </el-table-column>
                <el-table-column prop="transDate" label="产品对象" align="center" >
                    <template slot-scope="{row}">
                        {{row.productObjectNo}}{{row.productDesc}}
                    </template>
                </el-table-column>
            ...
            <div class="text_title">账单摘要明细(业务类型级)</div>
            <el-table
                v-loading="mainDetail_config.loading"
                :data="mainDetail_tableData"
                class="mbtm20"
                fit
                highlight-current-row
                style="width: 100%;"
                border
                @current-change="mainDetail_handleChange"
                @row-click="mainDetail_chooseList"
            >
                <el-table-column width="50">
                    <template slot-scope="scope">
                    <el-radio v-model="mainDetail_curRowRadio" :label="scope.row.businessTypeCode">
                        <span class="el-radio__label"></span>
                    </el-radio>
                    </template>
                </el-table-column>
                <el-table-column prop="transCurrDesc" label="业务类型" align="center" width="180">
                    <template slot-scope="{row}">
                        {{row.businessTypeCode}}{{row.businessDesc}}
                    </template>
                </el-table-column>
                <el-table-column prop="transCurrDesc" label="业务项目" align="center" width="210">
                    <template slot-scope="{row}">
                        {{row.businessProgramNo}}{{row.programDesc}}
                    </template>
                </el-table-column>

                <el-table-column prop="productObjectCode" label="产品对象" align="center">
                    <template slot-scope="{row}">
                        {{row.productObjectCode}}{{row.productDesc}}
                    </template>
                </el-table-column>
                ...
                <el-table-column prop="" label="操作" align="center" width="200">
                    <template slot-scope="{row}">
                        <el-button size="mini" type="primary" @click="mainDetail_handle1(row)">详情</el-button>
                        <el-button size="mini" type="primary" @click="mainDetail_handle2(row)">账单分期</el-button>
                    </template> 
                </el-table-column>
            </el-table>
            <pagination v-show="mainDetail_config.total>0" :total="mainDetail_config.total" :page.sync="mainDetail_config.pageNum" :limit.sync="mainDetail_config.pageSize" @pagination="getMainDetailList(row)" />
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="billMain_dialog = false">
                    关闭
                </el-button>
            </div>
        </el-dialog>
        ...
        <!-- 账单摘要明细(业务类型级) 详情 弹窗 -->
        <el-dialog title="账单摘要详情（业务类型级）" :visible.sync="mainDetail_dialog"  width="85%">
            <el-form :model="mainDetailObj">
                <el-form-item class="cl_td bnone" label="业务类型 " prop="businessTypeCodeTrans">
                    <el-input v-model="mainDetailObj.businessTypeCodeTrans" class="wd300" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="产品对象" prop="productObjectCodeTrans">
                    <el-input v-model="mainDetailObj.productObjectCodeTrans" class="wd300" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="业务项目" prop="businessProgramNoTrans">
                    <el-input v-model="mainDetailObj.businessProgramNoTrans" class="wd300" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="账单日期" prop="billDate">
                    <el-input v-model="mainDetailObj.billDate" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td bnone" label="账单金额" prop="postingAmount">
                    <el-input v-model="mainDetailObj.postingAmount" class="wd200" type="text" />
                </el-form-item>
            </el-form>
            <div  class="text_title">交易明细</div>
            <el-table
            ...
            <pagination v-show="config.total>0" :total="config.total" :page.sync="config.pageNum" :limit.sync="config.pageSize" @pagination="getList(mainDetailObj)" />
            ...
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="mainDetail_dialog = false">
                    关闭
                </el-button>
            </div>
        </el-dialog>
        ...
        <!-- 账单分期 弹窗 -->
        <el-dialog title="账单分期" :visible.sync="billStage_dialog"  width="85%">
            <stage-bill
                :transactionInfo="billStageObj"
                :showChoseBtn="showChoseBtn"
                @currentChange="billStage_currentChange"
                @rowClick="billStage_rowClick"
            ></stage-bill>
            <div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="billStage_dialog = false">
                    关闭
                </el-button>
            </div>
        </el-dialog>
...
import StageBill from "@/views/stageAndLoan/stagingBills/stagingBills.vue";
...
export default {
    ...
    data() {
        return {
            ...
            mainDetail_tableData: [],
            mainDetail_curRowRadio: '',
            mainDetail_curRowObj : {},
            // 分期 弹窗
            billStage_dialog: false,
            billStageObj: {},
            showChoseBtn: false,
            // 账单摘要明细(业务类型级) 详情
            mainDetail_dialog: false,
            mainDetailObj: {},
    ...
    methods: {
        ...
        // 查询
        hadleSearch() {
            this.getbillList() // 获取账单列表
            // this.getList()
            // this.getCustSystemUnitNo() // 获得查询客户的系统单元，非登录用户
        },
        // 获取账单列表
        getbillList() {
            this.bill_config.loading = true
            let params = Object.assign({}, this.bill_config, this.queryForm )
            this.getTableList('/nonfi/nonfinanceService/BSS.IQ.03.0003', params ,(res) => {
                if (res.returnCode == '000000') {
                    this.showInfo = true
                    this.bill_config.loading = false
                    if (res.returnData && res.returnData.rows && res.returnData.rows.length > 0 ) {
                        let rows =  res.returnData.rows
                        rows.forEach(item => {
                            item.businessProgramNo_concat = item.businessProgramNo+' ' +item.programDesc
                            item.productObjectCode_concat = item.productObjectCode+' ' +item.productDesc
                        })
                        this.bill_tableData = rows
                        this.bill_config.total = res.returnData.totalCount
                    } else {
                        this.bill_tableData = []
                    }
                }
        ...
        // 已出账单列表 详情 按钮
        bill_handleDetail(row) {
            console.log(row)
            this.billMain_dialog = true
            this.billMainObj = Object.assign({}, row)
            this.delay_tableData = this.billMainObj.page.rows
            this.getMainDetailList(row)
        },

        // 账单摘要明细(业务类型级) 当前行选中 
        mainDetail_handleChange(val) {
            console.log(val)
            this.mainDetail_curRowObj = val
        },
        mainDetail_chooseList(val) {
            console.log(val)
            this.mainDetail_curRowRadio = val.businessTypeCode
        },
        // 账单摘要明细(业务类型级) 列表
        getMainDetailList(row) {
            let params = {
            ...
            params = Object.assign(params, this.mainDetail_config, this.queryForm )

            this.getTableList('/nonfi/nonfinanceService/BSS.IQ.03.0004', params ,(res) => {
                if (res.returnCode == '000000') {
                    this.mainDetail_config.loading = false
                    if (res.returnData && res.returnData.rows && res.returnData.rows.length > 0 ) {
                        let rows =  res.returnData.rows
                        // rows.forEach(item => {
                        //     item.businessProgramNo_concat = item.businessProgramNo+' ' +item.programDesc
                        //     item.productObjectCode_concat = item.productObjectCode+' ' +item.productDesc
                        // })
                        this.mainDetail_tableData = rows
                        this.mainDetail_config.total = res.returnData.totalCount
                    } else {
                        this.mainDetail_tableData = []
                    }
                }
            })
        },
        // 账单摘要明细(业务类型级) 详情 按钮
        mainDetail_handle1(row) {
            console.log(row)
            this.mainDetail_dialog = true
            this.mainDetailObj = Object.assign({}, row)
            this.mainDetailObj.businessTypeCodeTrans = this.mainDetailObj.businessTypeCode + ' ' + this.mainDetailObj.businessDesc
            this.mainDetailObj.productObjectCodeTrans = this.mainDetailObj.productObjectCode + ' ' + this.mainDetailObj.productDesc
            this.mainDetailObj.businessProgramNoTrans = this.mainDetailObj.businessProgramNo + ' ' + this.mainDetailObj.programDesc
            this.getList(row)
            this.showTable = true
        ...
        // 账单摘要明细(业务类型级) 账单分期 按钮
        mainDetail_handle2(row) {
            console.log(row)
            let params = {
                customerNo: row.customerNo,
            ...
            this.post('/nonfi/nonfinanceService/ILS.IQ.01.0001', params ,(res) => {
                if (res.returnCode == '000000') {
                    if (res.returnData && res.returnData.rows && res.returnData.rows.length > 0 ) {
                        this.billStageObj = Object.assign(params, this.queryForm)
                        this.billStageObj.billAmt = res.returnData.rows[0].billAmt;
                        this.billStageObj.ecommEntryId = this.queryForm.externalIdentificationNo
                        this.billStageObj.ecommTransPostingCurr = row.currencyCode
                        
                        this.billStage_dialog = true
                    } 
                }
            })
        },
        ...
        // 账单分期 弹窗
        // 选择事件
        billStage_currentChange(val) {

        },
        billStage_rowClick(val) {
    ...
    transStageDetailBtn(e) {
        ...
        let params = {
                ...
                postingCurrencyCode: e.postingCurrencyCode,
                occurrDate: e.occurrDate,
                postingAmount: e.postingAmount,
