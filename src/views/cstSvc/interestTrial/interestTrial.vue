...
<template>
    <div class="allContent">
        ...
        <el-form ref="transForm" :model="queryForm">
            ...
            <el-form-item class="cl_td" label="试算日期" prop="beginDate">
                <el-date-picker
                    v-model="queryForm.interestStartDate"
                    class="wd200"
                    type="date"
                    placeholder="选择日期"
        ...
        <div v-if='isShow'>
            <el-form ref="accountRef" :model="accountForm">
                ...
                <el-form-item class="cl_td" label="账户代码" prop="accountId">
                    <el-input v-model="accountForm.accountId" class="wd190" type="text" />
                </el-form-item>
                <el-form-item class="cl_td" label="币种" prop="currencyCode">
                    <el-input v-model="accountForm.currencyCode" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td" label="利息累计日期" prop="interestStartDate">
            ...
            <!--利息累计信息-->
            <div class="text_title">余额单元信息</div>
            <el-table v-loading="listLoading1" :data="interestInfoList" class="mbtm20 " border fit highlight-current-row style="width: 100%;" row-key="id" lazy :load="loadTable" :tree-props="{children: 'children', hasChildren: 'haveChild'}" @current-change="handleList1Change" @row-click="chooseList1">
                ...
                <el-table-column prop="" label="币种" align="center" width="100">
                ...
                <el-table-column prop="prevInterestBillingDate" label="上一结息日" align="center" width="120"/>
                <el-table-column prop="nextInterestBillingDate" label="下一结息日" align="center" />
                <el-table-column prop="interest" label="试算金额" align="center" />
            </el-table>
          </div>
        <!--基本账户信息-->
        ...
        methods: {
            ...
            handleSearch() {
                if(!this.queryForm.externalIdentificationNo && !this.queryForm.idType && !this.queryForm.idNumber) {
                    ...
                    } else if(!this.queryForm.interestStartDate){
	                	this.$message({
	                        message: '请输入试算日期',
	                        type: 'warning'
	                    })
            ...
            // 主账户查询列表
            getList() {
            	console.log(this.queryForm.interestStartDate)
            ...
            //利息试算
            getInterestTrial() {
            	if(!this.curRow.currencyCode){
            		this.$message('请至少选中一条')
            		return
            	}
        		this.isShow = true
               	let obj = {
                    ...
                    interestStartDate: this.queryForm.interestStartDate,
                   
                }
                this.getTableList('/nonfi/nonfinanceService/BSS.IQ.01.0085', obj, (res) => {
                    if(res.returnCode == '000000') {
