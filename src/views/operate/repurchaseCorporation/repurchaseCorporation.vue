    ...
    <div class="allContent">
        ...
        <div v-if="showLoop">
            ...
            <el-table v-loading="config1.loading" :data="tableData1" class="mbtm20" fit highlight-current-row style="width: 100%" border row-key="id" 
                ...
                <el-table-column prop="statusCodeDesc" label="资金法人ID" align="center" />
                ...
                <el-table-column prop="absRepoIndicatorDesc" label="回购法人标志" align="center" />
                <el-table-column prop="" label="操作" align="center" width = "350">
                    <template slot-scope="{row}">
                        <el-button size="mini" icon="el-icon-more" type="primary" @click="handleUp(row)" >回购标志维护</el-button>
                    </template> 
                </el-table-column>
            </el-table>
        ...
        <!--回购标志维护-->
        <el-dialog title="系统单元新增" :visible.sync="dialogFormVisibleUp" width="65%" height="50%">
            <el-form ref="dataFormUp" :rules="rulesUp" :model="tempUp" >
                <el-form-item class="cl_td" label="回购法人标志维护" prop="absRepoIndicator">
                    <el-select v-model="tempUp.absRepoIndicator" class="wd200" >
                    <el-option
                        v-for="(item, index ) in absRepoIndicatorOptions"
                        :key="index"
                        :label="item.detailDesc"
                        :value="item.codes"/>
                    </el-select>
        ...
        <div v-if="showTrade">
           <div class="text_title">交易账户信息表</div>
            <el-table v-loading="config2.loading" :data="tableData2" class="mbtm20" fit  highlight-current-row style="width: 100%"
                ...
                <el-table-column prop="loanTypeDesc" label="业务类型" align="center" width="100" />
                <el-table-column prop="accountId" label="账户代码" align="center"  width="230" />
                ...
                <el-table-column prop="" label="操作" align="center" width = "350">
                    <template slot-scope="{row}">
                        <el-button size="mini" icon="el-icon-more" type="primary" @click="handleDetail(row)" >回购标志维护</el-button>
                    </template> 
                </el-table-column>
            </el-table>
...
import MccInfo from '@/components-ider/cstSvc/MccInfo'
    ...
    data() {
        return {
            ...
            absRepoIndicatorOptions: [],//回购法人标志维护
            // 账户列表
    ...
    mounted() {
        ...
        //回购法人标志维护
        this.getSelectDict( "dic_repurchaseMark").then(res => {
            this.absRepoIndicatorOptions = res
        })
    ...
    getList1() {
        ...
        let params = {
            ...
            transParams: [
              "dic_accountOrganForm", "dic_businessDebitCreditCode", "dic_accStatusCode", 'dic_repurchaseMark', 'dic_assetAccType'], //查找数据字典所需参数
            transDict: [  "accountOrganForm", "businessDebitCreditCode", "statusCode",'absRepoIndicator' ,'accountType'], //翻译前后key
        };
    ...
    getList2() {
        ...
        let params = {
            ...
            externalIdentificationNo: this.queryForm.externalIdentificationNo,
            isTrans: true, //是否需要翻译数据字典
            transParams: ['dic_repurchaseMark', 'dic_assetAccType'], //查找数据字典所需参数
            transDict: ['absRepoIndicator' ,'accountType'], //翻译前后key
        };
    ...
    loadTable2(tree, treeNode, resolve) {
      setTimeout(() => {
        ...
        let childList = [];
        this.getTableList(
          "/nonfi/nonfinanceService/BSS.IQ.01.0355",
          params1,
          (res) => {
            if (res.returnCode == "000000") {
              if (
        ...
        createData(){
            let items = {
                absRepoIndicator: this.tempUp.absRepoIndicator,
                accountId: this.tempUp.accountId,
                currencyCode: this.tempUp.currencyCode,
                externalIdentificationNo: this.queryForm.externalIdentificationNo,
            }
            this.post('/nonfi/nonfinanceService/BSS.UP.01.0019', items, (res) => {
                if (res.returnCode == '000000') {
                    this.dialogFormVisibleUp = false
                    this.$notify({
                    title: 'Success',
                    message: '确认回购法人标志维护成功',
                    type: 'success',
                    duration: 2000
                    })
                    this.getList2() 
