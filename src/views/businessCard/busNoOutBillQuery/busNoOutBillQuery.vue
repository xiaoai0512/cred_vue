...
<template>
    <div class="allContent">
        ...
        <div v-if="showBusines">
            ...
            <pagination  v-show="businessTotal > 0" :total="businessTotal" :page.sync="businessPage.pageNum"  :limit.sync="businessPage.pageSize" @pagination="getBusinessList" />
        </div>
        <!-- 选择业务项目 -->
        <div v-show="showBusItem">
            <div  class="text_title">选择业务项目</div>
            <el-form ref="busItemRef" :rules="busItemRules" :model="busItemObj">
                <el-form-item class="cl_td" label="币种" prop="currencyCode">
                    <!-- <el-input v-model="busItemObj.currencyCode" class="wd200" type="text" /> -->
                    <el-select clearable  v-model="busItemObj.currencyCode" class="wd200">
                        <el-option 
                ...
                <el-form-item class="cl_td" label="业务项目" prop="businessProgramNo">
                    <el-select clearable  v-model="busItemObj.businessProgramNo" class="wd200">
                        <el-option 
                        v-for="(item, index) in businessProgramNoOptions" 
                        :key="index" 
                        :label="item.businessProgramNo" 
                        :value="item.businessProgramNo" />
                    </el-select>
                </el-form-item>
            </el-form>
            <div class="cl_tr text_center">
                <el-button size="medium" icon="el-icon-search" type="primary" @click="hadleSearch2()">查询</el-button>
            </div>
        </div>

        <!-- 未出账单信息 -->
        <div v-show="showNObill">
            <div  class="text_title">未出账单信息</div>
            <el-form :model="noBillObj">
                <el-form-item class="cl_td" label="起始时间" prop="startDate">
                    <el-input v-model="noBillObj.startDate" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td" label="结束时间" prop="endDate">
                    <el-input v-model="noBillObj.endDate" class="wd200" type="text" />
                </el-form-item>
                <el-form-item class="cl_td" label="未出账单金额" prop="totalPostingAmount">
                    <el-input v-model="noBillObj.totalPostingAmount" class="wd200" type="text" />
                </el-form-item>
            </el-form>
            ...
            <el-table-column prop="" label="操作" align="center" width = "350">
                <template slot-scope="{row}">
                    <el-button size="mini" type="primary"
                        v-if=" row.globalSerialNumbrRelative != null && row.globalSerialNumbrRelative != ''"
                        @click="handle1(row)">关联交易</el-button>
                    <el-button size="mini" type="primary" @click="handle2(row)">同源交易</el-button>
        ...
        <pagination v-show="config.total>0" :total="config.total" :page.sync="config.pageNum" :limit.sync="config.pageSize" @pagination="queryDateAndList" />
        </div>
		<!-- 关联交易 -->
        <div v-if="showTable1" >
            <div class="text_title">关联交易列表</div>
            <el-table v-loading = "config1.loading" v-show="showTable1" :data="tableData1" class="mbtm20" fit  highlight-current-row style="width: 100%;" border>
                <el-table-column prop="externalIdentificationNo" label="外部识别号/账户号" align="center" width = "180">
                    <template slot-scope="{row}">
                        <span v-if="!row.externalIdentificationNo">
                            {{row.accountId}}
        ...
        <div v-if="showTable2">
            ...
            <el-table
                ...
                v-show="showTable2" :data="tableData2" class="mbtm20" fit highlight-current-row style="width: 100%;" border>
                <el-table-column prop="externalIdentificationNo" label="外部识别号/账户号" align="center" width = "180">
                    <template slot-scope="{row}">
                        <span v-if="!row.externalIdentificationNo">
                            {{row.accountId}}
        ...
        <!-- 交易信息弹窗 -->
        <el-dialog title="交易信息" :visible.sync="transInfo_dialog"  width="85%">
            <trans-info
                ...
                @stagingAccInfoBtn="stagingAccInfoBtn"></trans-info>
			<div slot="footer" class="dialog-footer">
                <el-button type="primary" @click="transInfo_dialog = false">关闭</el-button>
            </div>
        </el-dialog>
		<!-- 部分退货 弹窗 -->
               ...
               <el-form-item class="cl_td" label="分期期数" prop="ecommInstallmentPeriod" 
                ...
                <el-form-item class="cl_td" label="入账币种" prop="postingCurrencyCode">
                    <el-select v-model="transStageObj.postingCurrencyCode" class="wd200" clearable disabled>
                        <el-option
                        v-for="(item, index ) in currencyCodeOptions01"
                        :key="index"
                        :label="item.detailDesc"
                        :value="item.codes"
                        />
...
export default {
    ...
    mounted() {
        ...
        //  // 分期期数
         ...
         this.getSelectDict('dic_curreny').then((res) => {
            this.currencyCodeOptions01 = res
        })
         // 费用收取方式
    ...
    methods: {
        ...
        queryCurr() {
            ...
            this.post('/nonfi/nonfinanceService/BSS.IQ.03.0006', params ,(res) => {
                if (res.returnCode == '000000') {
                    this.showBusItem = true
                   // if (res.returnData && res.returnData.rows &&  res.returnData.rows.length == 1) {
                        this.currencyCodeOptions = res.returnData.rows
                        console.log(this.currencyCodeOptions)
                        this.queryBusItem() // 查询业务项目
                   // }
        ...
        handleInfo(row){
        	this.showBusines = false
        	this.showBusItem = true
        	this.queryForm.externalIdentificationNo = this.businessList[0].externalIdentificationNo
        ...
        queryBusItem() {
            ...
            this.post('/nonfi/nonfinanceService/BSS.IQ.03.0006', params ,(res) => {
                if (res.returnCode == '000000') {
                    if (res.returnData && res.returnData.rows &&  res.returnData.rows.length == 1) {
                        this.businessProgramNoOptions = res.returnData.rows
                        this.busItemObj.businessProgramNo = res.returnData.rows[0].businessProgramNo;
                    }else {
						this.busItemObj.businessProgramNo = '';
					}
                } 
        ...
        // 第二个查询按钮
        hadleSearch2() {
            this.queryDateAndList()
        },
        // 查询未出账单起始结束时间 和 未出账单列表
        queryDateAndList() {
            let params = {
                businessProgramNo : this.busItemObj.businessProgramNo,
		        currencyCode : this.busItemObj.currencyCode,
		        queryDate : true,
            }
            params = Object.assign(params, this.config, this.queryForm, params)
            this.getTableList('/nonfi/nonfinanceService/BSS.IQ.03.0006', params ,(res) => {
                if (res.returnCode == '000000') {
                    this.showBusItem = true
                    this.showNObill = true
                    if (res.returnData.obj == null ) {
                        this.noBillObj.totalPostingAmount = '0';//金额
                    }else {
						this.noBillObj.startDate = res.returnData.obj.startDate;
					    this.noBillObj.endDate = res.returnData.obj.endDate;
					    this.noBillObj.totalPostingAmount = res.returnData.obj.totalPostingAmount;//定义页面上的总汇展示
					}
                    // 未出账单列表
        ...
        getList1(row) {
            ...
            let params = {
                globalSerialNumbr : row.globalSerialNumbrRelative
            }
        ...
        getList2(row) {
            ...
            this.config2 = Object.assign(this.config2, params, this.queryForm)
            this.getTableList('/nonfi/nonfinanceService/ISS.IQ.03.0010', this.config2 ,(res) => {
                if (res.returnCode == '000000') {
    ...
    getTrailStageList() {
        ...
        this.trailStage_config = Object.assign(this.trailStage_config, params)
        this.getTableList('/card/cardService/ILS.IQ.01.0005', this.trailStage_config ,(res) => {
            if (res.returnCode == '000000') {
    ...
    getStageAccList(e) {
        ...
        this.stageAcc_config = Object.assign(this.stageAcc_config, params)
        this.getTableList('/nonfi/nonfinanceService/ILS.IQ.01.0020', this.stageAcc_config ,(res) => {
            if (res.returnCode == '000000') {
    ...
    getStageAccDetailList(row) {
        ...
        this.stageAccDetai_config = Object.assign(this.stageAccDetai_config, params)
        this.getTableList('/nonfi/nonfinanceService/ILS.IQ.01.0015', this.stageAccDetai_config ,(res) => {
            if (res.returnCode == '000000') {
